<template>
    <b-card no-body>
        <b-card-header class="border-0 header-sec">
            ...
            <b-button  variant="success" class="create_btn" v-b-modal.modal-1 @click="redirect()">Create Session</b-button>
        </b-card-header>
        <!-- {{ users }} -->
...
<!-- 
        ...
        <el-table-column label="Actions" min-width="180">
            <template slot-scope="scope">
                ...
                <base-button size="sm" type="info" v-b-modal.modal-1 @click="viewProgramModal(scope.row)">
                    <i class="ni ni-atom"></i>
                </base-button>
            
                <base-button size="sm" type="warning" @click="updateProgramModal(scope.row.id)">
...
export default {
  ...
  data() {
    ...
    //   propsToSearch: ['name', 'email', 'user_id', 'phone'],  // Fields to search
      tableColumns: [
        { prop: 'mainprogram.name', label: 'Program', minWidth: 140, sortable: true },
        { prop: 'name', label: 'Session', minWidth: 140, sortable: true },
        // { prop: 'status', label: 'Status', minWidth: 140, sortable: true },
        { prop: 'program_type', label: 'Type', minWidth: 140, sortable: true },
      ],
      programData:{
        name: '',
        status: 1
  ...
  methods: {
    ...
    async programList() {
      try {
        const response = await axios.get(`${process.env.VUE_APP_API_BASE_URL}ProgramList`);

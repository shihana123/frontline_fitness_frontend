<template>
    <div>
        <b-card no-body>
            ...
            <b-row class="mb-2 ml-1 mr-1">
                <b-col lg="4">
                    <base-input label="Select Trainer">
                        <select class="form-control" v-model="filterData.sales_id" required @change="changeFilter()">
                ...
                <b-col lg="4">
                    <base-input label="Status">
                        <select class="form-control" v-model="filterData.status" required @change="changeFilter()">
            ...
            <b-row class="mb-2 ml-1 mr-1">
                <b-col lg="4">
                    <h5>Task Count: {{ this.totalCount }}</h5>
                </b-col>
            </b-row>
            ...
            @selection-change="selectedRows = $event">
                ...
                <el-table-column label="Status" min-width="180">
                    <template slot-scope="scope">
                        ...
                        <p v-if="scope.row.status == false" class="text-error">Pending</p>
                        <p v-else-if="scope.row.status == true && scope.row.connected == true" class="text-success">Completed</p>
                        <p v-else-if="scope.row.status == true && scope.row.connected == false" class="text-warning">Not Connected , call postponed to {{ scope.row.postponed_date }}</p>
                    </template>
                </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        statuses: ['All', 'Pending', 'Completed', 'Connected', 'Not Connected'],
        totalCount: 0
      };
    },
    computed: {
    ...
    watch: {
        ...
        sales(newVal) {
            if (newVal && newVal.length > 0) {
                this.filterData.sales_id = newVal[0].id; // auto-select first
                this.taskList();
            }
        },
    },
    methods:{
        ...
        {
            try {
            const response = await axios.get(`${process.env.VUE_APP_API_BASE_URL}byrole/2/`);
            console.log(response);
            this.sales = response.data.filter(sale => sale !== null);
        ...
        async taskList(){
            ...
            axios.get(`${process.env.VUE_APP_API_BASE_URL}taskListbyuser/${this.filterData.sales_id}/${this.filterData.date}/${this.filterData.status}`, {

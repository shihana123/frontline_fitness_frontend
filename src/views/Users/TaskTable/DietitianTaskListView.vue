                        ...
                        @selection-change="selectedRows = $event">
                            ...
                            <el-table-column label="Actions" min-width="180">
                                <template slot-scope="scope">
                                    ...
                                    <base-button v-if="scope.row.task_type == 'dietition_only'"   size="sm" type="info" v-b-modal.modal-1  @click="updateDietTask(scope.row.id)">
                                        Update
                                    </base-button>
                                    
                                    <base-button v-else   size="sm" type="info" v-b-modal.modal-1  @click="updateTask(scope.row.task_type, scope.row.entity_data.id)">
                                        Update
                                    </base-button>
        ...
        <b-modal id="modal-1" size="lg" title="Update Task" hide-footer v-if="showModal" @close="showModal = false">
            <b-form @submit.prevent="updateDailyTask">
                ...
                <div class="pl-lg-12">
                    <b-row>
                        <b-col lg="6">
                            <base-input label="Need Meeting *">
                                <select class="form-control" v-model="updateData.need_meeting" required>
  ...
  export default {
    ...
    methods:{
        ...
        updateTask(task_type, client_id)
        {
            ...
            if (task_type == 'TDC' || task_type == 'Renewal' || task_type == 'dietchart' || task_type == 'dietition_only') {
                this.$router.push({ name: 'clients/consultation_details', params: { id: client_id } });
            } else if(task_type == 'First Trainer Consultation' || task_type == 'First Dietitian Consultation') {
        ...
        {  
            ...
            formData.append('meeting_id', this.selectedID);
            formData.append('need_meeting', this.updateData.need_meeting);
            formData.append('notes', this.updateData.notes);

            axios.post(`${process.env.VUE_APP_API_BASE_URL}updateMeeting`, formData,{
            headers: { Authorization: `Token ${token}` }
            })
            .then(response => {
